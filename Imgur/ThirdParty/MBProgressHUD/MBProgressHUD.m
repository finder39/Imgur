...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view labelText:(NSString*)labelText animated:(BOOL)animated{
	...
	[hud show:animated];
    hud.dimBackground = YES;
    if(labelText) hud.labelText = labelText;
    return MB_AUTORELEASE(hud);
}


+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view labelText:(NSString*)labelText detailLabelText:(NSString*)detailLabelText animated:(BOOL)animated{
	MBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
    hud.dimBackground = YES;
	[hud show:animated];
    if(labelText) hud.labelText = labelText;
    if(detailLabelText) hud.detailsLabelText = detailLabelText;
    return MB_AUTORELEASE(hud);
...
- (void)drawRect:(CGRect)rect {
	...
	CGRect allRect = self.bounds;
//    CGRect allRect = CGRectMake(self.bounds.origin.x, self.bounds.origin.y, self.bounds.size.width, self.bounds.size.width);
